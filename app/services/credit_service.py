...
class CreditPricing:
    ...
    _DEFAULT_PRICING = {
        "transcription_base": 1.0,        # 1 kredi/dakika
        "speaker_recognition": 0.5,       # 0.5 kredi/dakika (resemblyzer/silero backend)
        "speaker_diarization": 0.25,      # 0.25 kredi/dakika (pyannote.audio 3.1 Modal GPU)
        ...
        "assemblyai_speech_understanding_per_minute": 1.2,  # Speech Understanding (full features for English)
        "assemblyai_llm_gateway": 3.0,    # LLM Gateway fixed cost
        "entity_detection_per_minute": 0.03,  # Entity Detection only (all languages)
    }
